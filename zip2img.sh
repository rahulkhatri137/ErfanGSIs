...
echo "-> Create Temp and out dir" > /dev/null 2>&1
outdir="$LOCALDIR/out"
if [ ! "$2" == "" ]; then
    outdir="$(realpath $2)"
fi
...
if [[ $MAGIC == "OPPOENCRYPT!" ]] || [[ "$romzipext" == "ozip" ]]; then
    echo "-> ozip detected" > /dev/null 2>&1
...
if [[ $(echo "$romzip" | grep kdz) ]]; then
    echo "-> KDZ detected" > /dev/null 2>&1
...
if [[ $(echo "$romzip" | grep -i ruu_ | grep -i exe) ]]; then
    echo "-> RUU detected" > /dev/null 2>&1
    cp "$romzip" $tmpdir
...
if [[ $(7z l -ba "$romzip" | grep system.new.dat) ]]; then
    echo "-> Aonly OTA detected" > /dev/null 2>&1
    for partition in $PARTITIONS; do
        ...
        ls | grep "\.new\.dat" | while read i; do
            ...
            echo "-> Extracting $partition" > /dev/null 2>&1
    ...
    echo "-> nb0 type detected" > /dev/null 2>&1
    ...
    echo "-> Chunk detected" > /dev/null 2>&1
    for partition in $PARTITIONS; do
    ...
    echo "-> Image detected" > /dev/null 2>&1
    ...
    echo "-> Sin type detected" > /dev/null 2>&1
    ...
    echo "-> Bin images detected" > /dev/null 2>&1
    7z x -y "$romzip" 2>/dev/null >> $tmpdir/zip.log
    ...
    echo "-> P suffix images detected" > /dev/null 2>&1
    for partition in $PARTITIONS; do
    ...
    echo "-> Sign images detected" > /dev/null 2>&1
    7z x -y "$romzip" 2>/dev/null >> $tmpdir/zip.log
    for partition in $PARTITIONS; do
        [[ -e "$tmpdir/$partition.img" ]] && mv "$tmpdir/$partition.img" "$outdir/$partition.img"
    done
    ...
    echo "-> Super detected" > /dev/null 2>&1
    ...
    echo "-> Image zip firmware detected" > /dev/null 2>&1
    ...
    echo "-> Huawei UPDATE.APP detected" > /dev/null 2>&1
...
for partition in $PARTITIONS; do
    ...
    if [[ $EXT4PARTITIONS =~ (^|[[:space:]])"$partition"($|[[:space:]]) ]] && [ -f "$outdir"/$partition.img ]; then
        ...
        if [[ $(echo "$MAGIC" | grep "MOTO") ]]; then
            ...
            echo "-> Moto header detected on $partition in $offset" > /dev/null 2>&1
        elif [[ $(echo "$MAGIC" | grep "ASUS") ]]; then
            echo "-> Asus header detected on $partition in $offset" > /dev/null 2>&1
        else
            offset=0
        fi
        if [ ! $offset == "0" ]; then
